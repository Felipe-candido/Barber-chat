...
func NewService(shopID uuid.UUID, name, description string, durationMinutes int, priceCents int64) (Service, error) {
	...
	return Service{ID: uuid.New(), ShopID: shopID, Name: name, Description: strings.TrimSpace(description),
		DurationMinutes: durationMinutes, PriceCents: priceCents, Currency: "BRL", Active: true}, nil
}
