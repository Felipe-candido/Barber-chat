...
O link público usa slug da barbearia. Um formulário progressivo em formato de chat coleta nome, sobrenome e telefone; apresenta serviços, data e horários por profissional; permite revisar e confirmar; mostra os dados da reserva e solicita envio assíncrono. Cliente não possui conta. Dados incompletos permanecem no frontend e só são persistidos na confirmação. O frontend de demonstração está em `frontend-chat`, com Next.js e Tailwind, ainda sem integração com a API.
...
O backend está em `backend-chat` e o frontend em `frontend-chat`, com ferramentas Node próprias. Os caminhos do backend neste documento são relativos a `backend-chat`. Veja o [ADR 0008](adr/0008-backend-frontend-directories.md) para a organização e os imports Go. O [ADR 0004](adr/0004-business-modules.md) registra os limites de negócio; o [ADR 0005](adr/0005-explicit-module-layers.md) define as camadas explícitas atuais. A organização se apoia nas [convenções oficiais de pacotes e comandos Go](https://go.dev/doc/modules/layout); as fronteiras de negócio são decisões específicas deste produto.
